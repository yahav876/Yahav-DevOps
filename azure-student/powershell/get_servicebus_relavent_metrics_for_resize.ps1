...
try {
    ...
    Get-AzSubscription | Where-Object { ($_.Name -match ".*") -and ($_.State -eq 'Enabled') } | ForEach-Object {
        ...
        foreach ($sbid in $getServiceBus) {
            $sizeMetrics = Get-AzMetric -ResourceId $sbid.Id -MetricName "Size" -StartTime $datenow.AddDays(-30) -EndTime $datenow -AggregationType "Maximum" -TimeGrain 01:00:00 -WarningAction SilentlyContinue
            $metricList = @()
            foreach ($metric in $sizeMetrics.Data.Maximum) {
                if ($metric -gt $max) {
                    $max = $metric
                    # Write-Output $max
                    if (($max/1000) -lt 256) {$metricList += $metric}
                        $x = ($metricList | measure -Maximum).Maximum
                        $x
                        # $sbid.Id
            ...
            $cpuMetrics =  Get-AzMetric -ResourceId $sbid.Id -MetricName "NamespaceCpuUsage" -StartTime $datenow.AddDays(-30) -EndTime $datenow -AggregationType "Maximum" -TimeGrain 01:00:00 -WarningAction SilentlyContinue -ErrorAction SilentlyContinue
            if ($cpuMetrics) {
