...
for p in "${PROFILES[@]}"
    ...
    OUTPUT=`aws lambda list-functions --function-version ALL --region=$r --profile=$p --output text --query "Functions[?Runtime=='nodejs12.x'].FunctionArn"`
    echo $OUTPUT
    done
done
